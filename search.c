...
void thesearch(char* mda, char* theting){
	char l = strlen(mda);
	for(int i = 0; i < l; i++){
		if( (mda + i) == 'm'){
			Movie *pm;
			pm = movie;
			if ( strcmp(pm -> title, theting) == 0){
				printf("%d:%s:%s:%s:%d:%d:", pm->serialNumber, pm->title->name, pm->genre->name, pm->director->name, pm->year, pm->runTime);
				pm->actorCur = pm->actor;

				printf("%s", pm->actorCur->name);

				while (pm->actorCur->next != NULL)
				{
					pm->actorCur = pm->actorCur->next;
					printf(", %s", pm->actorCur->name);
				}
			}
			while( pm -> next != NULL ){
			...
			Director *pd;
			pd = director;
			if ( strcmp(pd -> title, theting) == 0){
				printf("%d:%s:%c:%s:", pd->serialNumber, pd->director->name, pd->sex, pd->birth->name);

				pd->titleCur = pd->title;

				printf("%s", pd->titleCur->name);

				while (pd->titleCur->next != NULL)
				{
					pd->titleCur = pd->titleCur->next;
					printf(", %s", pd->titleCur->name);
				}			}
			while( pd -> next != NULL ){
				pd = pd -> next;
			...
			Actor *pa;
			pa = actor;
			if ( strcmp(pa -> title, theting) == 0){
				printf("%d:%s:%c:%s:", pa->serialNumber, pa->actor->name, pa->sex, pa->birth->name);

				pa->titleCur = pa->title;

				printf("%s", pa->titleCur->name);

				while (pa->titleCur->next != NULL)
				{
					pa->titleCur = pa->titleCur->next;
					printf(", %s", pa->titleCur->name);
				}			}
			while( pa -> next != NULL ){
				pa = pa -> next;
...
void update(char* mda, char* option, char* num) {
	int optLen = strlen(option) + 1, actorCnt = 0, cnt = 0;
